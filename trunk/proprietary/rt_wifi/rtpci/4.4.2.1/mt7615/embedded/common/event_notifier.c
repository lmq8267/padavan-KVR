...
VOID WHCMlmePeriodicExec(PRTMP_ADAPTER pAd)
{
    ...
    UINT32 ChLoadDetectPeriod = 10; //Default
    ...
    concurrent_bands = HcGetAmountOfBand(pAd);
    if(pEventNotifyCfg->ChLoadDetectPeriod > 0)
    ...
    if (IS_MT7615(pAd)) 
    {
        ...
        {
            if(IsRun2G && (RcGetBandIdxByRf((HD_CFG*)pAd->pHdevCfg, RFIC_24GHZ) == i))
            {
                for (j=0;j<WDEV_NUM_MAX;j++) 
                {
                    p2GWdev = pAd->wdev_list[j];
                    if (!p2GWdev)
                        continue;
                    
                    if (HcGetBandByWdev(p2GWdev) == i)
                        break;
                ...
                if(p2GWdev)
                {
                    pChload->ch_load_2g.bValid = TRUE;
                    pChload->ch_load_2g.Channel = p2GWdev->channel;
                    pChload->ch_load_2g.ChannelBusyTime += pAd->OneSecMibBucket.ChannelBusyTime[i];   
                }
            }
            else if(IsRun5G && (RcGetBandIdxByRf((HD_CFG*)pAd->pHdevCfg, RFIC_5GHZ) == i))
            {
                for (j=0;j<WDEV_NUM_MAX;j++) 
                {
                    p5GWdev = pAd->wdev_list[j];
                    if (!p5GWdev)
                        continue;
                    
                    if (HcGetBandByWdev(p5GWdev) == i)
                        break;
                ...
                if(p5GWdev)
                {
                    pChload->ch_load_5g.bValid = TRUE;
                    pChload->ch_load_5g.Channel = p5GWdev->channel;
                    pChload->ch_load_5g.ChannelBusyTime += pAd->OneSecMibBucket.ChannelBusyTime[i];
                }
            }
        }
        
        if((pEventNotifyCfg->ChLoadRound % ChLoadDetectPeriod) == 0)
        {
            pEventNotifyCfg->ChLoadRound = 0;
            pChload->type = WHC_DRVEVNT_CHANNEL_LOAD_REPORT;
            
            if(p2GWdev)
                if_dev = p2GWdev->if_dev;
            else if(p5GWdev)
                if_dev = p5GWdev->if_dev;

            if(if_dev)
                RtmpOSWrielessEventSend(if_dev, RT_WLAN_EVENT_CUSTOM, WHC_DRVEVNT_CHANNEL_LOAD_REPORT,
                               NULL, (PUCHAR)pChload, sizeof(*pChload));
        }
    }
}
