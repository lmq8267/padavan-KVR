	...
	IN UINT32 key_deri_alg)
{
	...
	if (!get_mic_len_by_alg(key_deri_alg, &mic_len)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s:get mic len fail(alg = %d)\n", __func__, key_deri_alg));
		return;
	}
	...
	IN BOOLEAN          bDataFrameFirst)
{
	...
	if (pEntry) {
		...
		if (pEntry->SecConfig.PmfCfg.UsePMFConnect == TRUE
			&& tr_ctl->tr_entry[pEntry->wcid].PortSecured == WPA_802_1X_PORT_SECURED) {
			ULONG	TmpLen;
	...
	OUT UCHAR *rsn_len)
{
	...
	if (ElememtId == SEC_RSNIE_WPA1_IE) {
		...
		if (IS_AKM_WPA2(pSecConfig->AKMMap) && pSecConfig->ft_only == FALSE) {
			...
			NdisMoveMemory(pRsnie_auth->auth[0].oui + 4 * AkmCnt, OUI_WPA2_AKM_FT_8021X, 4);
			AkmCnt++;
		}
		...
		if (IS_AKM_WPA2PSK(pSecConfig->AKMMap) && pSecConfig->ft_only == FALSE) {
			...
			NdisMoveMemory(pRsnie_auth->auth[0].oui + 4 * AkmCnt, OUI_WPA2_AKM_FT_PSK, 4);
			AkmCnt++;
		}
	...
	OUT UCHAR *rsn_len)
{
	...
	if (pSecConfig->bIsWPA2EntOSEN) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s: No PMKID in RSN as OSEN is enabled \n", __func__));
		return TRUE;
	}
#endif
	if (ElememtId == SEC_RSNIE_WPA2_IE) {
	...
	IN PMAC_TABLE_ENTRY pEntry)
{
	...
	case WDEV_TYPE_GC:
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> %s(p2p)\n", __func__));
		break;
#endif /* P2P_SUPPORT */
	}
	...
	*Offset = 0;
	...
	while (len > sizeof(RSNIE3)) {
		...
		if (pEid->Eid == IE_RSN) {
			...
			*Offset += (pEid->Len + 2);
		} else if (pEid->Eid == IE_RSNXE) {
			UINT status_code;

			status_code = parse_rsnxe_ie(&pEntry->SecConfig, (UCHAR *)pEid, pEid->Len + 2, FALSE);
			is_parse_rsnxe = TRUE;
	...
	if (IS_AKM_SAE(pEntry->SecConfig.AKMMap) &&
		(pEntry->SecConfig.is_h2e_connect || pEntry->SecConfig.rsnxe_len != 0) &&
		!is_parse_rsnxe) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			("%s: should include rsnxe in 4-way msg2/3, rsnxe_len = %d, is_h2e_connect = %d\n", __func__, pEntry->SecConfig.rsnxe_len, pEntry->SecConfig.is_h2e_connect));
		result = FALSE;
	}
#endif
	...
	IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (pEntry->wdev)
		AsicGetTxTsc(pAd, pEntry->wdev, pEntry->wdev->SecConfig.Handshake.RSC);
...
/* Cache the PMK against PMKID, generated in process_ecdh_element() */
	...
	if (IS_AKM_OWE(pSecConfig->AKMMap)) {
		...
		if (owe->pmkid) {
			sta_add_pmkid_cache(pAd, pEntry->Addr, owe->pmkid, pSecConfig->PMK, pmk_len, pStaCfg->wdev.func_idx, wdev);
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (IS_AKM_WPA2(pSecConfig->AKMMap)
		...
		if ((!IS_AKM_OPEN(GET_SEC_AKM(pSecConfig))) && tr_entry->PortSecured == 1) {
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("PTK:"));

			for (i = 0; i < 64; i++)
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%02x", pSecConfig->PTK[i]));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (!wdev) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: 2-way fail due to wdev is null!\n", __func__));
		return;
	}
	...
	if (!pEntry) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: 2-way fail due to pEntry is null!\n", __func__));
		return;
	}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WpaEAPOLRetryAction ===>\n"));
		...
		if (IS_AKM_PSK(pSecConfig->AKMMap) || IS_AKM_1X(pSecConfig->AKMMap)) {
			if (pHandshake->MsgType == EAPOL_PAIR_MSG_1) {
				...
				if (pHandshake->MsgRetryCounter > GROUP_MSG1_RETRY_LIMIT) {
					pHandshake->GTKState = REKEY_FAILURE;
					MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s::Group rekey timeout from %02X:%02X:%02X:%02X:%02X:%02X\n", __func__, PRINT_MAC(pHandshake->SAddr)));
				} else {
