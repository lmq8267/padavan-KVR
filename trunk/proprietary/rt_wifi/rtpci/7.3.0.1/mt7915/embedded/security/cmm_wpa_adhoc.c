	...
	OUT	PUCHAR			pTxTsc)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): Not support for HIF_MT yet!\n",
				 __func__));
		return;
	}
	...
	Wcid = pEntry->wdev->bss_info_argument.bmc_wlan_idx;
	/* Read IVEIV from Asic */
	offset = iveiv_tb_base + (Wcid * iveiv_tb_size);
	NdisZeroMemory(IvEiv, 8);
	NdisZeroMemory(pTxTsc, 6);
	RTMP_IO_READ32(pAd->hdev_ctrl, offset, &IV);
	RTMP_IO_READ32(pAd->hdev_ctrl, offset + 4, &EIV);
	...
	*(pTxTsc + 5) = EIV & 0xff000000;
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s(): WCID(%d) TxTsc 0x%02x-0x%02x-0x%02x-0x%02x-0x%02x-0x%02x\n",
			 __func__, Wcid, *pTxTsc, *(pTxTsc + 1), *(pTxTsc + 2), *(pTxTsc + 3), *(pTxTsc + 4), *(pTxTsc + 5)));
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Adhoc_WpaEAPOLStartAction ===>\n"));
	pHeader = (PHEADER_802_11)Elem->Msg;

	/*For normaol PSK, we enqueue an EAPOL-Start command to trigger the process. */
	if (Elem->MsgLen == 6)
		pEntry = MacTableLookup2(pAd, Elem->Msg, Elem->wdev);
	else
	...
	if (pEntry) {
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" PortSecured(%d), WpaState(%d), AuthMode(%d), is_cache(%d)\n",
				 tr_entry->PortSecured, pAuthenticator->WpaState, pEntry->AuthMode, is_pmkid_cache_in_sec_config(&pEntry->SecConfig)));
		...
		if ((tr_entry->PortSecured == WPA_802_1X_PORT_NOT_SECURED)
			...
			&& ((pEntry->AuthMode == Ndis802_11AuthModeWPAPSK)
				...
				|| ((pEntry->AuthMode == Ndis802_11AuthModeWPA2) && (is_pmkid_cache_in_sec_config(&pEntry->SecConfig))))) {
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Adhoc_WpaEAPOLKeyAction ===>\n"));
	pHeader = (PHEADER_802_11)Elem->Msg;
	...
	*((USHORT *)&peerKeyInfo) = cpu2le16(*((USHORT *)&peerKeyInfo));
	...
	do {
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Receive EAPoL-Key frame from STA %02X-%02X-%02X-%02X-%02X-%02X\n", PRINT_MAC(pEntry->Addr)));

		if (eapol_len > Elem->MsgLen - LENGTH_802_11 - LENGTH_802_1_H) {
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("The length of EAPoL packet is invalid\n"));
			break;
		}
		...
		if (((pEapol_packet->ProVer != EAPOL_VER) && (pEapol_packet->ProVer != EAPOL_VER2)) ||
			...
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Key descripter does not match with WPA rule\n"));
			break;
		}
		...
		if ((pEntry->WepStatus == Ndis802_11TKIPEnable) && (peerKeyInfo.KeyDescVer != KEY_DESC_TKIP)) {
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Key descripter version not match(TKIP)\n"));
			break;
		}
		...
		/* either the pairwise or the group cipher is AES-CCMP for Key Descriptor 2. */
			...
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Key descripter version not match(AES)\n"));
			break;
		}
	...
	IN ULONG			TimeInterval)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> Adhoc_WpaStart4WayHS\n"));

	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_HALT_IN_PROGRESS)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[ERROR]WPAStart4WayHS : The interface is closed...\n"));
		return;
	}

	if (!pEntry) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[ERROR]WPAStart4WayHS : The entry doesn't exist.\n"));
		return;
	}
	...
	pBssid =  pStaCfg->Bssid;
	group_cipher = pStaCfg->GroupCipher;
	/* delete retry timer */
	RTMPCancelTimer(&pAuthenticator->MsgRetryTimer, &Cancelled);

	if (pBssid == NULL) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[ERROR]WPAStart4WayHS : No corresponding Authenticator.\n"));
		return;
	}

	/* Check the status */
	if ((pAuthenticator->WpaState > AS_PTKSTART) || (pAuthenticator->WpaState < AS_INITPMK)) {
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[ERROR]WPAStart4WayHS : Not expect calling=%d\n", pAuthenticator->WpaState));
		return;
	}
	...
	if (pEntry->AuthMode == Ndis802_11AuthModeWPA2PSK) {
		...
		NdisMoveMemory(&PMK_key[8],  pStaCfg->wdev.if_addr, MAC_ADDR_LEN);
		NdisMoveMemory(&PMK_key[14], pEntry->Addr, MAC_ADDR_LEN);
		RT_HMAC_SHA1(pStaCfg->PMK, LEN_PMK, PMK_key, 20, digest, LEN_PMKID);
	...
	pAuthenticator->WpaState = AS_PTKSTART;
	os_free_mem(mpool);
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== Adhoc_WpaStart4WayHS: send Msg1 of 4-way\n"));
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> Adhoc_PeerPairMsg1Action\n"));
	...
	pCurrentAddr = pStaCfg->wdev.if_addr;
	pmk_ptr = pStaCfg->PMK;
	group_cipher = pStaCfg->GroupCipher;
	rsnie_ptr = pStaCfg->RSN_IE;
	rsnie_len = pStaCfg->RSNIE_Len;
	...
	RTMPToWirelessSta(pAd, pEntry,
					  ...
					  (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED) ? FALSE : TRUE);
	os_free_mem(mpool);
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== PeerPairMsg1Action: send Msg2 of 4-way\n"));
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> Adhoc_PeerPairMsg2Action\n"));
	...
	gtk_ptr = pStaCfg->GTK;
	group_cipher = pStaCfg->GroupCipher;
	default_key = pStaCfg->wdev.DefaultKeyId;

	if (pStaCfg->wdev.AuthMode == Ndis802_11AuthModeWPA2PSK) {
		rsnie_len = pStaCfg->RSNIE_Len;
		rsnie_ptr = pStaCfg->RSN_IE;
	}
	...
	Adhoc_AsicGetTxTsc(pAd, BSS0, pEntry, TxTsc);
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== PeerPairMsg2Action: send Msg3 of 4-way\n"));
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("===> Adhoc_PeerPairMsg3Action\n"));
	...
	if (pEntry->AuthMode == Ndis802_11AuthModeWPA2PSK ||
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerPairMsg3Action: AuthMode(%s) PairwiseCipher(%s) GroupCipher(%s)\n",
	...
	os_free_mem(mpool);
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== PeerPairMsg3Action: send Msg4 of 4-way\n"));
}
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> Adhoc_PeerPairMsg4Action\n"));
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	PSTA_ADMIN_CONFIG   pStaCfg = NULL;
#endif
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("===> Adhoc_PeerGroupMsg1Action\n"));
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerGroupMsg1Action: AuthMode(%s) PairwiseCipher(%s) GroupCipher(%s)\n",
			 GetAuthMode(pEntry->AuthMode),
			 GetEncryptType(pEntry->WepStatus),
			 GetEncryptType(group_cipher)));
	...
	if ((pAd->OpMode == OPMODE_STA) && INFRA_ON(pStaCfg) &&
	...
	os_free_mem(mpool);
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<=== PeerGroupMsg1Action: send group message 2\n"));
}
	...
	IN MAC_TABLE_ENTRY  *pEntry)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> Adhoc_Wpa4WayComplete\n"));
	...
	pAuthenticator = &pEntry->WPA_Authenticator;
	pSupplicant = &pEntry->WPA_Supplicant;
	compare_address = NdisCmpMemory(pStaCfg->wdev.if_addr, pEntry->Addr, MAC_ADDR_LEN);
	...
	/* Indicate Connected for GUI */
	pAd->IndicateMediaState = NdisMediaStateConnected;
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Adhoc_Wpa4WayComplete - WPA2, AuthMode(%d)=%s, WepStatus(%d)=%s, GroupWepStatus(%d)=%s\n\n",
			 pEntry->AuthMode, GetAuthMode(pEntry->AuthMode),
			 pEntry->WepStatus, GetEncryptType(pEntry->WepStatus),
			 pStaCfg->GroupCipher,
			 GetEncryptType(pStaCfg->GroupCipher)));
}
	...
	IN PVOID SystemSpecific3)
{
	...
	if ((pEntry) && IS_ENTRY_CLIENT(pEntry)) {
		...
		ASSERT(pStaCfg);
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Adhoc_WPARetryExec---> ReTryCounter=%d, WpaState=%d\n", pAuthenticator->MsgRetryCounter, pAuthenticator->WpaState));
		...
		case Ndis802_11AuthModeWPA2PSK:
			if (pAuthenticator->MsgRetryCounter == 0) {
				...
				if (pStaCfg->WscControl.bWscTrigger == FALSE)
#endif /* IWSC_SUPPORT // */
				{
					MlmeDeAuthAction(pAd, pEntry, REASON_4_WAY_TIMEOUT, FALSE);
					MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Adhoc_WPARetryExec::MSG1 timeout\n"));
				}
				...
				if ((pAuthenticator->WpaState == AS_PTKSTART) || (pAuthenticator->WpaState == AS_INITPSK) || (pAuthenticator->WpaState == AS_INITPMK)) {
					MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Adhoc_WPARetryExec::ReTry MSG1 of 4-way Handshake\n"));
					Adhoc_WpaStart4WayHS(pAd, pEntry, PEER_MSG1_RETRY_EXEC_INTV);
				}
				...
				{
					MlmeDeAuthAction(pAd, pEntry, REASON_4_WAY_TIMEOUT, FALSE);
					MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Adhoc_WPARetryExec::Retry MSG3, TIMEOUT\n"));
				}
			}
	...
	OUT PEAPOL_PACKET       pMsg)
{
	...
	*(USHORT *)(pKeyInfo) = cpu2le16(*(USHORT *)(pKeyInfo));
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> ConstructEapolMsg for %s %s\n", ((bWPA2) ? "WPA2" : "WPA"), GetEapolMsgType(MsgType)));
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("	     Body length = %d\n", CONV_ARRARY_TO_UINT16(pMsg->Body_Len)));
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("	     Key length  = %d\n", CONV_ARRARY_TO_UINT16(pKeyDesc->KeyLength)));
}
