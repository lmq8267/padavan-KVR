...
caddy_dl() {
       ...
       if [ ! -f "$caddy_dir/caddy/caddy_filebrowser" ]; then
		...
		curl -L -k -o "$caddy_dir/caddy/caddy_filebrowser" --connect-timeout 10 --retry 3 "${proxy}https://github.com/lmq8267/padavan-KVR/blob/main/trunk/user/caddy/caddy_filebrowser" || wget --no-check-certificate -O "$caddy_dir/caddy/caddy_filebrowser" "${proxy}https://github.com/lmq8267/padavan-KVR/blob/main/trunk/user/caddy/caddy_filebrowser"
		if [ "$?" = 0 ] ; then
			...
			if [ $(($($caddy_dir/caddy/caddy_filebrowser -h | wc -l))) -gt 3 ] ; then
				...
				break
	   			...
	   			logger -t "【caddy】" "下载不完整，删除...请手动下载 ${proxy}https://github.com/lmq8267/padavan-KVR/blob/main/trunk/user/caddy/caddy_filebrowser 上传到  $caddy_dir/caddy/caddy_filebrowser"
